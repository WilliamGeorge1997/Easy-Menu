                            ...
                            <div class="card-body">
                                ...
                                <div class="app-brand justify-content-center">
                                    <a href="index.html" class="app-brand-link gap-2">
                                        ...
                                        <span
                                            class="app-brand-text demo text-heading fw-bold">{{ __('dashboard/login.header') }}</span>
                                    </a>
                                </div>
                                <!-- /Logo -->
                                <h4 class="mb-1">{{ __('dashboard/login.welcome') }}</h4>
                                <p class="mb-6">{{ __('dashboard/login.sign_in_message') }}</p>

                                <form id="formAuthentication" class="mb-6" action="{{ route('admin.login') }}"
                                    method="POST">
                                    @csrf

                                    <div class="mb-6">
                                        <label for="email" class="form-label">{{ __('attributes.email') }}</label>
                                        <input type="text" autocomplete="on" class="form-control" id="email"
                                            name="email" placeholder="{{ __('placeholders.email') }}" autofocus
                                            value="{{ old('email') }}" />
                                        @error('email')
                                            <div>
                                    ...
                                    <div class="mb-6 form-password-toggle">
                                        <label class="form-label"
                                            for="password">{{ __('attributes.password') }}</label>
                                        ...
                                        <div class="d-flex justify-content-between">
                                            <div class="form-check mb-0">
                                                <input class="form-check-input" type="checkbox" id="remember-me"
                                                    name="remember_me" @checked(old('remember_me')) value="1" />
                                                <label class="form-check-label"
                                                    for="remember-me">{{ __('attributes.remember_me') }}</label>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="mb-6">
                                        <button class="btn btn-primary d-grid w-100 btn-loader"
                                            type="submit">{{ __('dashboard/login.sign_in') }}</button>
                                    </div>
                                </form>
